...
clean:
	rm -f $(addsuffix .hex, $(TARGETS)) $(addsuffix .elf, $(TARGETS)) $(addprefix src/, $(addsuffix .o, $(TARGETS))) $(OBJECTS) src/lib/hausbus_protocols.o src/lib/pwm.o
...
%.elf:
	...
	echo "$(PUR)Building pwm handler$(NC)"
	make --always-make src/lib/pwm.o
	echo "$(PUR)Building other objects$(NC)"
	make objects
	echo "$(PUR)Building $(basename $@) binary$(NC)"
	$(COMPILE) -std=gnu99 -o $@ $(OBJECTS) src/$(basename $@).o src/lib/hausbus_protocols.o src/lib/pwm.o
